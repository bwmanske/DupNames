...
bool CIniFile::DoesSectionExist(string SectionName, string FileName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool CIniFile::SetValue(string KeyName, string Value, string SectionName, string FileName)
{
	...
	if (Load(FileName, content))											// Make sure the file is loaded
	{
		if (!DoesSectionExist(SectionName, FileName))							// If the Section doesn't exist
		{
...
bool CIniFile::DoesFileExist(string FileName)
{
	ifstream f(FileName.c_str());
	return f.good();
}

bool CIniFile::DoesPathExist(string path)
{
	struct stat info;

	int returnVal = 0;   // debug only

	int statRC = stat(path.c_str(), &info);
	if (statRC != 0) {
		if (errno == ENOENT)  { returnVal = 1; return false; }  // something along the path does not exist
		if (errno == ENOTDIR) { returnVal = 2; return false; }  // something in path prefix is not a dir
		returnVal = 3;
		return false;
	}
